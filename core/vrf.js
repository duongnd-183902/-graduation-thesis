...
function hashToCurve(pk, input) {
    let rv = newCandidateSecp256k1Point(utils.solidityPack(['uint256', 'uint256', 'uint256', 'uint256'], [HASH_TO_CURVE_HASH_PREFIX.toString(), pk[0].toString(), pk[1].toString(), input.toString()]));
    while (!isOnCurve(rv)) {
...
function encodePacked(hash,pk,gamma,u,v){
    return utils.solidityPack(['uint256','uint256','uint256','uint256','uint256','uint256','uint256','uint256','uint256','address'], [SCALAR_FROM_CURVE_POINTS_HASH_PREFIX.toString(),hash[0].toString(),hash[1].toString(),pk[0].toString(),pk[1].toString(),gamma[0].toString(),gamma[1].toString(),v[0].toString(),v[1].toString(),u]);
}
